...
async function main() {
  ...
  const teamRows = await db
    .insert(teamMembers)
    .values([
      { name: "Marte Solberg", email: "marte.solberg@nordbygg.no", role: "Procurement", initials: "MS" },
      { name: "Jonas Vik", email: "jonas.vik@nordbygg.no", role: "Project management", initials: "JV" },
      { name: "Ida Bremnes", email: "ida.bremnes@nordbygg.no", role: "Contract administration", initials: "IB" },
      { name: "Henrik Aune", email: "henrik.aune@nordbygg.no", role: "Finance", initials: "HA" },
      { name: "Camilla Fossum", email: "camilla.fossum@nordbygg.no", role: "Procurement", initials: "CF" },
    ])
    .returning();
  ...
  const companyDefs = [
    { company: "Nordvik Bygg AS", orgNr: "923609016", category: "Groundworks", owner: "Marte Solberg" },
    { company: "Fjellstrand Elektro AS", orgNr: "984851006", category: "Electrical", owner: "Jonas Vik" },
    { company: "Hansteen Ventilasjon AS", orgNr: "916879067", category: "HVAC", owner: "Ida Bremnes" },
    { company: "Sørlandet Rørservice AS", orgNr: "985399077", category: "Plumbing", owner: "Henrik Aune" },
    { company: "Bergheim Transport AS", orgNr: "914778271", category: "Logistics", owner: "Camilla Fossum" },
    { company: "Kvernland Stillas AS", orgNr: "917422575", category: "Scaffolding", owner: "Marte Solberg" },
    { company: "Lysaker Maskin AS", orgNr: "981585378", category: "Heavy equipment", owner: "Jonas Vik" },
    { company: "Storøy Malerservice AS", orgNr: "912660680", category: "Painting", owner: "Ida Bremnes" },
    { company: "Trondheim Grunnteknikk AS", orgNr: "989522951", category: "Geotechnical", owner: "Henrik Aune" },
    { company: "Vestby Sikkerhet AS", orgNr: "920268082", category: "Site security", owner: "Camilla Fossum" },
  ];
  ...
  await db.insert(tasks).values([
    { title: "Request updated insurance certificate", subcontractorId: byCompany("Hansteen Ventilasjon AS").id, eventId: eventByType("Insurance document expires soon").id, ownerId: byName("Ida Bremnes").id, due: new Date("2026-08-12"), priority: "HIGH", status: "IN_PROGRESS", comment: "2 comments" },
    { title: "Review new management registration", subcontractorId: byCompany("Nordvik Bygg AS").id, eventId: eventByType("Managing director changed").id, ownerId: byName("Marte Solberg").id, due: new Date("2026-08-14"), priority: "NORMAL", status: "NOT_STARTED", comment: "1 comment" },
    { title: "Clarify registered charge with supplier", subcontractorId: byCompany("Lysaker Maskin AS").id, eventId: eventByType("New registered charge identified").id, ownerId: byName("Jonas Vik").id, due: new Date("2026-08-18"), priority: "NORMAL", status: "WAITING_FOR_INFORMATION", comment: "3 comments" },
    { title: "Document review of 2025 accounts", subcontractorId: byCompany("Sørlandet Rørservice AS").id, eventId: eventByType("Annual accounts submitted").id, ownerId: byName("Henrik Aune").id, due: new Date("2026-08-21"), priority: "LOW", status: "IN_PROGRESS" },
    { title: "Confirm board composition in contract file", subcontractorId: byCompany("Kvernland Stillas AS").id, eventId: eventByType("Board member changed").id, ownerId: byName("Marte Solberg").id, due: new Date("2026-08-25"), priority: "LOW", status: "NOT_STARTED" },
    { title: "Archive updated address details", subcontractorId: byCompany("Bergheim Transport AS").id, eventId: eventByType("Company address changed").id, ownerId: byName("Camilla Fossum").id, due: new Date("2026-07-28"), priority: "LOW", status: "COMPLETED", comment: "1 comment" },
  ]);

  await db.insert(notes).values([
    { subcontractorId: byCompany("Nordvik Bygg AS").id, authorId: byName("Marte Solberg").id, text: "Spoke with the new managing director. Ongoing assignments continue with the same site management, and the project plan for Grefsen is unchanged.", createdAt: new Date("2026-05-15") },
    { subcontractorId: byCompany("Sørlandet Rørservice AS").id, authorId: byName("Henrik Aune").id, text: "Reviewed the 2025 accounts together with finance. Revenue growth is in line with the reported order book. Operating result is slightly lower, noted for the next quarterly follow-up.", createdAt: new Date("2026-05-07") },
    { subcontractorId: byCompany("Lysaker Maskin AS").id, authorId: byName("Jonas Vik").id, text: "Registered charge relates to financing of two new excavators according to the supplier. Documentation requested for the contract file.", createdAt: new Date("2026-04-29") },
  ]);
